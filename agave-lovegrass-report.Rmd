...
suppressPackageStartupMessages(library(lattice))    # Q-Q plots
suppressPackageStartupMessages(library(car))        # Levene test homogeneity
...
# font_import()  # To import system fonts for use by R
...
**Note** This markdown document was used for investigating several types of analyses; development ceased on 20 July 2020. See individual scripts for specifics of data, analyses, and visualizations.
...
1. Survival of individual agave plants
...
# For plot & analyses, only consider a maximum of 3 agaves for any 
...
# Calculate probabilities for each of the treatments
# Create vector of all beta 1 coeffs; it is zero for control (ref) treatment
b_1 <- c(0, surv_single_coeffs[2:nrow(surv_single_coeffs), 2])
# e^(b0 + b1)
exp_sums <- exp(surv_single_coeffs[1, 2] + b_1)
surv_single_coeffs$probs <- exp_sums / (1 + exp_sums)

knitr::kable(x = surv_single_coeffs[, c(1, 2, 3, 5, 6)], 
             format = "markdown",
             row.names = FALSE,
             digits = 5,
             col.names = c("Predictor", "Coefficient Estimate", "Error", "P-value", "Survival Probability"))
```

In this table, the row listed as $(Intercept)$ represents the Control treatment.
...
if (nrow(surv_single_contrasts) > 0) {
  ...
  # Updated emmeans adds parentheses around treatment values, need to remove 
  # them to match with actual values
  surv_single_diffs <- gsub(x = surv_single_diffs,
                            pattern = ")",
                            replacement = "")
  surv_single_diffs <- gsub(x = surv_single_diffs,
                            pattern = "\\(",
...
_Note_: The p-values from Tukey _post-hoc_ tests will *always* be higher than p-value from the initial model. This is because the _post-hoc_ test is correcting for the fact that we are making many pairwise comparisons (we have to compare each treatment to every other treatment), and the chance that we encounter a false positive (inferring significance where the low p-value is due to chance alone) is increasing. In this case, there are `r (nrow(surv_single_coeffs) * (nrow(surv_single_coeffs) - 1)) / 2` total pairwise comparisons.

*UPDATE*: If the interest is _only_ comparing treatments to the Control, _post-hoc_ tests seem unnecessary. This is because, in the regression models, we are treating the Control as the "reference" category, and estimating coefficients for all treatments other than the Control (sort of; in reality, the Control treatment is effectively represented in the model by the intercept, $\beta_0$). In the table presenting the coefficient estimates, the p-value column indicates whether or not the estimate is significantly different from zero. In those cases where the estimate _is_ significantly different from zero (p < 0.05), it means the Treatment of interest had an effect significantly different from the reference category, which in this case is the control.


So.


As long as you are _only_ interested in comparing treatments to the Control, and not to one another, _post-hoc_ tests are not necessary. If, however, you want to know if Hand-pulling was significantly different from Shade, then _post-hoc_ tests are necessary.
...
message("Note, as of 2020-07-27, this figure is no longer saved; see agave-survival-boxplot.R.")
...
b_2 <- round(surv_single_coeffs[5, 2], digits = 3)
```
...
We can consider a similar comparison, between the Control treatment and the Javalina exclusion and shade treatment ("J + S"):

$$
p(Survival|Treatment = Javalina + Shade) = \frac{e^{`r b_0` + `r b_2`}}{1 + e^{`r b_0` + `r b_2`}}
$$

```{r j-plus-s-prob, echo = FALSE}
p_js <- exp(x = (surv_single_coeffs[1, 2] + surv_single_coeffs[5, 2])) / (1 + exp(x = surv_single_coeffs[1, 2] + surv_single_coeffs[5, 2]))
p_js <- round(p_js, digits = 3)
```

So the estimated probability of survival in the J + S treatment is `r p_js` and the difference in survival probability from the Control treatment is `r p_js` - `r p_c` = `r p_js - p_c`.

Note the explanations above do not discuss random effects of plot. So when reporting these values, it is best to say that this is the _average_ effect of treatment on survival.
...
##### Aside: Interpreting linear regression
In contrast to logistic regression, linear regression is a straighforward additive model:

$$
\# Leaves = \beta_0 + \beta_1 \times Treatment + b_0
$$

For this explanation, we will ignore the random intercept effect, $b_0$, so this formula becomes

$$
\# Leaves = \beta_0 + \beta_1 \times Treatment
$$

In our model, $\beta_0$ is the intercept of the model, and $\beta_1$ is the coefficient for the Treatment of interest. For the Control treatment, $\beta_1 = 0$, so the formula for the number of leaves of agaves in the Control treatment becomes 

$$
\# Leaves(Treatment = Control) = \beta_0
$$

or 

$$
\#Leaves(Treatment = Control) = `r round(size_single_coeffs[1, 2], digits = 3)`
$$

To calculate the agave size for any other treatment, we use the formula

$$
\# Leaves = \beta_0 + \beta_1
$$

substituting the $\beta_1$ with the coefficient of the Treatment of interest. For example, if we wanted to know the size of an agave in the Hand-pulling treatment, we substitute $\beta_0 = `r round(size_single_coeffs[1, 2], digits = 3)`$ and $\beta_1 = `r round(size_single_coeffs[2, 2], digits = 3)`$, for an average size of `r round(size_single_coeffs[1, 2] + size_single_coeffs[2, 2], digits = 3)` leaves. 


Another way of interpreting these coefficients is that they show the _difference_ in size from the control treatment. That is, the coefficient for the Hand-pulling treatment, `r round(size_single_coeffs[2, 2], digits = 3)`, is the difference in size between plants in the Control treatment and plants in the Hand-pulling treatment.
...
# Remove parentheses that may be introduced to treatment names in emmeans
size_single_contrasts$contrast <- gsub(x = size_single_contrasts$contrast,
                                       pattern = ")",
                                       replacement = "")
size_single_contrasts$contrast <- gsub(x = size_single_contrasts$contrast,
                                       pattern = "\\(",
...
message("Note, as of 2020-08-10, this figure is no longer saved; see agave-size-boxplot.R.")
...
```{r size-linearity}
plot(x = resid(size_single_model),
     y = agave_sizes$live_leaf_number,
     xlab = "Residuals",
     ylab = "# Leaves")
```
...
```{r size-homogeneity-levene}
leveneTest(residuals(size_single_model) ~ live_agave_data$Treatment)
```

```{r size_homogenety-visual}
plot(size_single_model,
     xlab = "Fitted values",
...
## 2. How do mechanical removal and agaves affect the percent of Lehmann cover?
...
### 2.1.a How do selected treatments and agave size affect percent cover?

```{r cover-data-live-prep, warning = FALSE}
# Need to reduce data to only treatments C and H
cover_data_live <- live_agave_data[live_agave_data$Treatment %in% c("C", "H"), ]
...
# Only 17 rows of data when H and C are included. Too few for mixed-effects
...
Here we are interested to know how certain treatments and the _size_ of the agave plants affect the percent cover of Lehman lovegrass. We are interested in both the Hand-pulling and Weed-eating treatments, but since there was only a single observation of a live agave in the Weed-eating treatments, only Control and Hand-pulling treatments are considered in 2.1.a (N = `r nrow(cover_data_live)`).
...
cover_size_plot <- ggplot(data = cover_data_live, 
  ...
  ggtitle(label = "Figure 2.1.a. Percent Lehmann cover by agave size") +
  theme_bw() +
  theme(text = element_text(family = "Times New Roman"))
print(cover_size_plot)
message("Note, as of 2020-08-17, this figure is no longer saved.")
...
Where $b_0$ is the random intercept for plot. However, when considering _only_ the samples from H and Control) treatments, there are too few samples (N = `r sum(!is.na(cover_data_live$live_leaf_number))`) to run a mixed effect model. A good old fashioned linear regression model is then:
...
_Note_: Similar to question 1.2, there are several H treatments with zero leaf counts. It may be necessary instead to evaluate a model that does not include the number of agave leaves.

#### 2.1.b How does agave size effect percent cover when treatment interacts with size?
This analysis is identical to the one in 2.1.a, above, but allows the relationship between the percent cover and the size of agaves to differ among treatments. That is, the previous model included only treatment and agave size:

$$
\% Lehmann \enspace cover = \beta_0 + \beta_1 \times Treatment + \beta_2 \times \# Leaves
$$

This model affords each treatment a different intercept in the relationship between percent cover and agave size; however, if we want to allow a model with different slopes among treatments, the formula would be:

$$
\% Lehmann \enspace cover = \beta_0 + \beta_1 \times Treatment + \beta_2 \times \# Leaves + \beta_3 \times Treatment \times \# Leaves
$$

```{r cover-analysis-interaction}
# Run cover model, with interaction between Treatment and size
cover_interaction_model <- lm(formula = areial_cover ~ Treatment * live_leaf_number, data = cover_data_live)
cover_interaction_summary <- summary(cover_interaction_model)
cover_interaction_coeffs <- cover_interaction_summary$coefficients
cover_interaction_coeffs <- as.data.frame(cover_interaction_coeffs)

# Move Treatments from row names into column
cover_interaction_coeffs <- cbind(Predictor = rownames(cover_interaction_coeffs),
                                  cover_interaction_coeffs)
rownames(cover_interaction_coeffs) <- NULL

# Update column names
colnames(cover_interaction_coeffs) <- c("Predictor", "Estimate", "Std.Error", "t.value", "p.value")

# Replace "Treatment" values
cover_interaction_coeffs$Predictor <- gsub(pattern = "TreatmentH",
                                           replacement = "Hand-pulling",
                                           x = cover_interaction_coeffs$Predictor)

# Replace live_leave_number with human-readable version
cover_interaction_coeffs$Predictor <- gsub(pattern = "live_leaf_number",
                                           replacement = "Agave size",
                                           x = cover_interaction_coeffs$Predictor)

# Ensure small p-values are not shown as zero
cover_interaction_coeffs$p.value <- as.character(ifelse(
  test = cover_interaction_coeffs$p.value < 1e-5,
  yes = signif(x = cover_interaction_coeffs$p.value, digits = 5),
  no = round(x = cover_interaction_coeffs$p.value, digits = 5)))

knitr::kable(x = cover_interaction_coeffs[, c(1, 2, 3, 5)], 
             format = "markdown",
             row.names = FALSE,
             digits = 5,
             col.names = c("Predictor", "Coefficient Estimate", "Error", "P-value"))
...
```{r cover-model-comparison}
# Is this model better than the additive alone model?
model_compare <- anova(cover_size_model, cover_interaction_model)
```

We should compare the performance of this model with the simpler model, which did not include an interaction term. The more complex model has one additional parameter ($\beta_3$, the interaction coefficient) and _is_ significantly better than the simple model (F = `r round(model_compare$F[2], digits = 3)`, p = `r round(model_compare[2, "Pr(>F)"], digits = 3)`). So we can be confident that the more complex model is a better fit and should be used for interpretation. Also because we have reduced treatment to only two levels (Control and Hand-pulling), _post-hoc_ tests are unnecessary.

```{r cover-plot-interaction}
# Plot the model results
# Predict lines of relationships
control_line <- data.frame(Treatment = "C",
                              live_leaf_number = control_limits)
control_line$areial_cover <- predict(cover_interaction_model, newdata = control_line)

hand_pulling_line <- data.frame(Treatment = "H",
                                   live_leaf_number = hand_pulling_limits)
hand_pulling_line$areial_cover <- predict(cover_interaction_model, newdata = hand_pulling_line)

# Plot the data
cover_interaction_plot <- ggplot(data = cover_data_live, 
                          mapping = aes(x = live_leaf_number, y = areial_cover,
                                        group = Treatment, fill = Treatment,
                                        shape = Treatment)) +
  geom_line(data = control_line, color = "#787878") +
  ...
  theme_bw() +
  theme(text = element_text(family = "Times New Roman"))
print(cover_interaction_plot)
message("Note, as of 2020-08-17, this figure is no longer saved; see lovegrass-cover-plot.R.")
# suppressMessages(expr = ggsave(filename = "output/cover-interaction-plot.pdf", plot = cover_interaction_plot))
```
...
```{r cover-linearity}
plot(x = resid(cover_interaction_model),
     y = cover_data_live$areial_cover,
     xlab = "Residuals",
     ylab = "Percent cover E. lehmanniana")
```

**Homogeneity of variance**

```{r cover-homogeneity-levene}
leveneTest(residuals(cover_interaction_model) ~ cover_data_live$Treatment)
```

```{r cover-homogenety-visual}
plot(x = predict(cover_interaction_model, type = "response"),
     y = resid(cover_interaction_model),
     xlab = "Fitted values",
     ylab = "Standardized residuals")
abline(h = 0, lty = 2)
```
...
```{r cover-normality}
plot(cover_interaction_model, which = 2)
```
...
```{r cover-pa-linearity}
plot(x = resid(cover_model_pa),
     y = cover_data_all$areial_cover,
     xlab = "Residuals",
     ylab = "Percent cover E. lehmanniana")
```
...
```{r cover-pa-homogeneity-levene}
leveneTest(residuals(cover_model_pa) ~ cover_data_all$Treatment)
```

```{r cover-pa-homogenety-visual}
plot(cover_model_pa,
     xlab = "Fitted values",
     ylab = "Standardized residuals")
```
...
```{r cover-pa-normality}
qqmath(cover_model_pa)
```
