...
```{r}
# TODO
# 1. Apply post-hoc test to 1.1a and 1.2a with `emmeans` package; see https://cran.r-project.org/web/packages/emmeans/vignettes/comparisons.html.
...
surv_single_coeffs <- bind_cols(data.frame(rownames(surv_single_summary$coefficients)),
                                data.frame(round(surv_single_coeffs, digits = 4)))
...
```{r results = "asis"}
if (any(surv_multi_coeffs$p.value < 1e-6)) {
  cat("\n_Note_: P-values reported as 0 are artifacts of rounding; they are not truly zero.")
}
```
