...
```{r}
# TODO
# 1. Apply post-hoc test to 1.1a and 1.2a with `lsmeans` package; see http://rcompanion.org/handbook/G_06.html.
# 2. Consider adding asterisks to indicate significant differences (from control?)
# 4. See if there is an easy way to add outliers to plots while using stat_summary instead of geom_boxplot
# 5. Check explanations
# 6. Fix p-values of 0.
# 8. Add example of logistic regression explanation of coefficient estimates
# 9. Update author info and add comments on MS
```
...
agave_count_plot <- ggplot(data = agave_counts, mapping = aes(x = Treatment, y = num_alive)) +
  ...
  ylab(label = "Number of live agaves") +
  ggtitle(label = "Figure 1.1. Agave survival by treatment") +
  geom_text(data = survive_text_counts,
            label = survive_text_counts$mean_survivorship, # referring to bare column won't work
...
$$
p(Survival|Treatment) = {e^{\beta_{0} + \beta_{1}Treatment}\over 1 + e^{\beta_{0} + \beta_{1}Treatment}}
$$
...
$$
{p(Survival|Treatment) \over 1 - p(Survival|Treatment)} = e^{\beta_{0} + \beta_{1}Treatment}
$$

Finally, we take the natural logarithm of both sides to get:
$$
ln \left({p(Survival|Treatment) \over 1 - p(Survival|Treatment)}\right) = \beta_{0} + \beta_{1}Treatment
$$
...
$$
p(Survival|Treatment = Control) = {e^{\beta_{0}}\over 1 + e^{\beta_{0}}}
$$
...
$$
p(Survival|Treatment = Control) = {e^{`r b_0`}\over 1 + e^{`r b_0`}}
$$
...
$$
p(Survival|Treatment = Hand-pulling) = {e^{\beta_{0} + \beta_{1}Treatment}\over 1 + e^{\beta_{0} + \beta_{1}Treatment}}
$$
...
$$
p(Survival|Treatment = Hand-pulling) = {e^{`r b_0` + `r b_1`}\over 1 + e^{`r b_0` + `r b_1`}}
$$
...
cover_pa_plot <- ggplot(data = cover_data_all, 
  ...
  xlab(label = "Agave") +
  ylab(label = "Percent cover") +
  ggtitle(label = "Figure 2.2. Percent Lehmann cover by agave presence") +
  theme_bw() +
  theme(text = element_text(family = "Times New Roman"))
print(cover_pa_plot)
