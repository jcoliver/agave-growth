...
1. How are cases when an agave was dead handled, especially for growth?
    + It looks like all agaves were included in analyses, rather than a maximum
    of three per row/plot/treatment combination.
2. With the boxplots of agave size and treatment, shade the boxes corresponding 
to survivorship in that treatment or add survivorship (of original five agaves) 
values near the boxes.
...
agave_count_plot <- ggplot(data = agave_counts, 
  ...
  ggtitle(label = "Agave survival by treatment") +
  theme_bw()
...
In this section, individual agave sizes (measured by number of leaves) is a continuous response variable in linear regression mixed-effects models. The data are restricted to cases where there was a live agave (N = `r nrow(live_agave_data)`).

```{r plot-size, warning = FALSE}
# Want a box plot with Treatment on x-axis, # leaves on y
agave_sizes <- agave_data %>%
  filter(Status == 1) %>% # Drop any dead plants
  select(Treatment, live_leaf_number) %>%
  mutate(live_leaf_number = as.integer(live_leaf_number))
...
agave_size_plot <- ggplot(data = agave_sizes, 
  ...
  xlab(label = "Treatment") +
  ylab(label = "# Leaves") +
  ggtitle(label = "Agave size by treatment") +
  theme_bw()
...
cover_size_plot <- ggplot(data = cover_data_live, 
  ...
  geom_point(size = 2.0, shape = 21, alpha = 0.8) +
  scale_fill_manual(values = c("#787878", "#D8B365", "#5AB4AC"),
                     labels = c("Control", "Hand-pulling", "Weed-eating")) +
  xlab(label = "# Leaves") +
  ylab(label = "% Cover") +
  ggtitle(label = "% Lehmann cover by agave size") +
  theme_bw()
print(cover_size_plot)
ggsave(filename = "output/cover-size-plot.pdf", plot = cover_size_plot)
```
...
cover_pa_plot <- ggplot(data = cover_data_all, 
  ...
  xlab(label = "Agave") +
  ylab(label = "% Cover") +
  ggtitle(label = "% Lehmann cover by agave presence") +
  theme_bw()
print(cover_pa_plot)
ggsave(filename = "output/cover-pa-plot.pdf", plot = cover_pa_plot)
```
