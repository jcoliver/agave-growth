...
# 4. Add post-hoc for 2.1
# 6. Try 2.1 model without any treatments other than Control
# 7. Do 2.1 with an interaction effect between treatment and agave size

# From A. Gill:
# 1) Post-hoc test for both the Lehmann lovegrass analyses. 
# 4) Including two more analysis/figures for the Lehmann competition with 
# agaves (with just the control treatment and no weed control treatment) 
...
#### 2.1.b How does agave size in control treatments effect percent cover?
This analysis is identical to the one in 2.1, above, but includes only plants from the Control treatment.

```{r control-cover-size}
control_data_live <- cover_data_live[cover_data_live$Treatment == "C", ]
control_size_model <- lm(formula = areial_cover ~ live_leaf_number, data = control_data_live)
control_size_summary <- summary(control_size_model)

control_size_coeffs <- control_size_summary$coefficients

# Coerce to a data frame
control_size_coeffs <- as.data.frame(control_size_coeffs)

# Predict the line of the relationship
control_limits <- c(min(control_data_live$live_leaf_number),
                    max(control_data_live$live_leaf_number))
control_line <- data.frame(Treatment = "C",
                              live_leaf_number = control_limits)
control_line$areial_cover <- predict(control_size_model, newdata = control_line)
rm(control_limits)

# Plot the data
control_size_plot <- ggplot(data = control_data_live, 
                          mapping = aes(x = live_leaf_number, y = areial_cover)) +
  geom_line(data = control_line, color = "#787878") +
  geom_point(size = 2.0, alpha = 0.75, fill = "#787878") +
  xlab(label = "Number of leaves") +
  ylab(label = "Percent cover") +
  ggtitle(label = "Figure 2.1. Percent Lehmann cover by agave size in Control Treatment") +
  theme_bw() +
  theme(text = element_text(family = "Times New Roman"))
print(control_size_plot)
suppressMessages(expr = ggsave(filename = "output/control-size-plot.pdf", plot = cover_size_plot))
