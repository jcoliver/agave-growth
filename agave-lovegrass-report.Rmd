...
# TODO
1. Change two boxplots to show 90% CI (0.05 & 0.95); see https://stackoverflow.com/questions/21310609/ggplot2-box-whisker-plot-show-95-confidence-intervals-remove-outliers
2. Apply post-hoc test with `lsmeans` package; see http://rcompanion.org/handbook/G_06.html. 
    + Which tests?
...
# Calculate 75% quantile of the number of live agaves for positioning of mean 
# survivorship text 
survival_whisker <- agave_counts %>%
  ungroup() %>%
  group_by(Treatment) %>%
  summarize(whisker_max = quantile(x = num_alive, prob = 0.75) +
              1.5 * (quantile(x = num_alive, prob = 0.75) -
              quantile(x = num_alive, prob = 0.25)))

# Merge with agave data so we can find values for whisker
survival_upper_whisker <- merge(x = agave_counts,
                                y = survival_whisker)

# Drop any values above whisker_max for each treatment, the find largest 
# remaining value
survival_upper_whisker <- survival_upper_whisker %>%
  filter(num_alive <= whisker_max) %>%
  group_by(Treatment) %>%
  summarise(upper_whisker = max(num_alive))
